@extends('layouts.backend.template_mikapi', ['title' => 'DHCP Lease'])
@push('csslib')
    <!-- DATATABLE -->
...
@section('content')
    <div class="row" id="cancel-row">
        ...
        @include('mikapi.dhcp.lease.detail')
...
@push('js')
    <script>
        const url_index = "{{ route('mikapi.dhcp.lease') }}" + param_router
        const url_index_api = "{{ route('api.mikapi.dhcp.leases.index') }}"
        const url_index_api_router = "{{ route('api.mikapi.dhcp.leases.index') }}" + param_router
        var id = 0
        var perpage = 50
    </script>
            ...
            columns: [{
                ...
                render: function(data, type, row, meta) {
                    if (type == 'display') {
                        ...
                        if (row.dynamic) {
                            text += '<span class="badge me-1 badge-info" title="Dynamic">D</span>'
                        }
                        if (row.radius) {
                            text += '<span class="badge me-1 badge-secondary" title="Eadius">R</span>'
                        }
                        text += `</div>`
                        return text
                    } else {
